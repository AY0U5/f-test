...
@Component({
  selector: 'app-dossier-projet-document-create-admin',
  templateUrl: './dossier-projet-document-create-admin.component.html'
})
export class DossierProjetDocumentCreateAdminComponent  implements OnInit {
   ...
   private _validDossierProjetDocumentLibelle = true;
    ...
    validateDossierProjetExigenceAppliques(){
    ...
    const isPermistted = await this.roleService.isPermitted('DossierProjet', 'add');
    if(isPermistted) {
    ...
    const isPermistted = await this.roleService.isPermitted('DossierProjetExigenceEtat', 'add');
    if(isPermistted) {
